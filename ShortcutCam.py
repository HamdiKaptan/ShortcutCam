...
#değiştirilicek kod
    ...
    # bulunan göcüklerin sayımı ve numara tespiti ile kısayol atanması
    if count_defects == 1:
        cv2.putText(img,"2", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
        # at.hotkey('ctrl', 'shift','z')
        ...
        #resim=at.screenshot("ekrangoruntusu.jpg")
    else:
        cv2.putText(img,"0", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)

    # analizleri gösterme
    cv2.imshow('kameram', img)
    all_img = np.hstack((drawing, crop_img))
    cv2.imshow('konturlenmis', all_img)
